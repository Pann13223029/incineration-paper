...
data/raw/facility_annual/*
  ...
  -> 05_fleet_decomposition.py
       separate participation, covered activity, design capacity, and endpoint composition
  ...
  -> 05b_scientific_revision.py
       fit revision-frozen entry models, support-aware risks, collinearity,
       event attacks, raw-kW models, and shared-control component decomposition
  -> 06_robustness.py
...
|:--|:--|:--|
| Fleet coverage and composition | FY2024: installed capacity 41.1% all/46.4% active; positive output 40.4% all/46.6% active; 80.1% throughput coverage; 70.5% design-capacity share. FY2005-FY2024 all-record prevalence rises 19.50 points versus 2.19 among endpoint-common lineages. | Count, flow, capacity, and composition remain distinct; endpoint-only lineages are not verified openings or closures. |
...
| Revised entry inference | 1,999 lineage bootstraps per frame; broad scale OR 6.72 (4.31-12.46); standardized annual risk 2.53 versus 16.66 per 1,000 at 100 and 300 t/day | Revision-frozen five-parameter model; 300 t/day is the 98.98th percentile, so 24/60/120 t/day predictions and temporal-collinearity diagnostics are required. |
| Generator components | 6,511 engineering-valid rows, 493 lineages; shared-control design/factor/utilization components exactly reproduce direct cohort log gaps | Lead with raw installed kW; do not combine the utilization-adjusted factor coefficients with the shared-control identity decomposition. |
...
| Identity and fleet | `facility_identity_audit.md`, `identity_low_margin_links.csv`, `linkage_validation_packet.csv`, `linkage_validation_protocol.md`, `fleet_decomposition.md`, `fleet_decomposition.csv`, `fleet_turnover_decomposition.md`, `fleet_turnover_decomposition.csv`, `fy2024_fleet_segments.csv` |
| Core models | `sample_definition.md`, `scientific_revision_results.md`, `revised_entry_results.csv`, `revised_entry_bootstrap.csv`, `revised_entry_influence.csv`, `revised_entry_robustness.csv`, `entry_standardized_risk.csv`, `entry_capacity_support.csv`, `entry_design_diagnostics.csv`, `entry_specification_summary.csv`, `entry_sample_flow.csv`, `entry_state_audit.csv`, `adoption_event_composition.csv`, `raw_quantity_component_results.csv`, `common_control_component_decomposition.csv`, `figure3_adjusted_components.csv`, `adoption_results.md`, `regression_results.md` |
