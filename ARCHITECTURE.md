...
1. **Extensive margin:** lagged observed first-adoption logit hazard on coded, initially non-generating facilities, with prior-year age-band indicators, prior-year capacity, year fixed effects, prefecture fixed effects, and facility-clustered standard errors; reported as average marginal effects, with lagged complementary log-log and lagged linear probability versions retained as robustness checks.
