...
More fundamentally, the ratio of within-to-total variance for log(efficiency) in this dataset is just 0.001 — meaning that 99.9% of the variation in efficiency across the sample is *between* facilities rather than *within* facilities over time. This is a substantively important finding in itself: it means that facilities do not meaningfully improve or decline in efficiency over their operational lives. But it also means that facility fixed effects would discard essentially all of the variation in the dependent variable, making estimation impossible and confirmed by the formal Hausman test. The random effects estimator, which exploits between-facility variation while modeling within-facility observations as correlated draws from a facility-specific distribution, is the appropriate panel estimator in this setting.

A formal Hausman test comparing random effects and fixed effects specifications was conducted; the result did not reject the random effects specification at conventional significance levels, consistent with the prior expectation that fixed effects estimation is inadvisable when within-facility variation in the outcome is negligible.
