...
Generated artifacts are written to `legacy/research/packets/dist/` and ignored in Git.
...
2. rebuilds `legacy/thesis/thesis.pdf` with `tectonic`, or reuses an already-current local `legacy/thesis/thesis.pdf` if `tectonic` is unavailable
...
2. creates a flattened supervisor-only handoff at `legacy/research/packets/dist/supervisor-handoff/`
3. writes `legacy/research/packets/dist/supervisor-handoff.zip`
4. refreshes the stable local alias `legacy/research/packets/latest-supervisor-handoff`

The packet and handoff commands work from the current thesis source plus the generated manifests and output artifacts already in the repo. They are packaging and verification commands, not full analysis rebuild commands. If `tectonic` is missing, they can still proceed when `legacy/thesis/thesis.pdf` already exists and is current relative to `thesis.tex` and the figure files.

Use loose PDFs only while drafting. Once the thesis is sendable, review the packet contents and then freeze the milestone with `npm run legacy:checkpoint:freeze`.
...
Use the stable alias `legacy/research/packets/latest-supervisor-handoff` when you only want the newest sendable supervisor bundle.
