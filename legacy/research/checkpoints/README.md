...
Generated artifacts are written to `legacy/research/checkpoints/dist/` and ignored in
Git. A stable local alias is also maintained at
`legacy/research/checkpoints/latest-sendable`, so you do not need to remember the
...
4. refreshes `legacy/research/checkpoints/latest-sendable` to point at the newest frozen checkpoint
5. prints a suggested git tag for that sendable state

Optional local tag creation:

```bash
python3 legacy/scripts/12_freeze_checkpoint.py --create-tag
```
...
- Use `legacy/research/checkpoints/latest-sendable` as the convenience path when you only want the newest frozen sendable state.
