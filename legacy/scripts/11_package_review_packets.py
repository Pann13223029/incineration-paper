...
PACKET_ROOT = REPO_ROOT / "legacy" / "research" / "packets" / "dist"
...
THESIS_DIR = REPO_ROOT / "legacy" / "thesis"
...
SUPERVISOR_FILES: list[tuple[str, str | None]] = [
    ("legacy/thesis/thesis.pdf", None),
    ("legacy/research/notes/executive-summary-for-supervisor.md", None),
    ("legacy/research/notes/examiner-risk-register.md", None),
    ("legacy/research/notes/what-this-thesis-does-not-claim.md", None),
...
SUBMISSION_FILES: list[tuple[str, str | None]] = [
    ...
    (".python-version", None),
    ("legacy/thesis/thesis.pdf", None),
    ("legacy/thesis/thesis.tex", None),
    ("legacy/research/notes/examiner-risk-register.md", None),
    ("code/analysis/panel_utils.py", None),
    ("code/analysis/05a_power_adoption.py", None),
    ("code/analysis/05_panel_regression.py", None),
    ("code/analysis/06_robustness.py", None),
    ("code/analysis/07_rebuild_analysis.py", None),
    ("code/analysis/08_verify_claims.py", None),
    ("legacy/scripts/14_generate_checkpoint_delta.py", None),
    ("output/checkpoint_delta.md", None),
    ("output/claim_verification.md", None),
    ("output/claim_evidence_map.md", None),
    ("output/sample_definition.md", None),
...
def ensure_current_state() -> None:
    run([sys.executable, "code/analysis/08_verify_claims.py"])
    run([sys.executable, "legacy/scripts/14_generate_checkpoint_delta.py"])
    try:
        tectonic = resolve_tectonic()
    except FileNotFoundError as exc:
        if thesis_pdf_is_current():
            print("tectonic not found; reusing existing current legacy/thesis/thesis.pdf")
            return
        raise FileNotFoundError(
            "tectonic not found and legacy/thesis/thesis.pdf is missing or stale relative to thesis sources"
        ) from exc
...
def build_packets() -> None:
    ...
    write_readme(
        ...
        metadata,
        [
            "1. Read `legacy/research/notes/executive-summary-for-supervisor.md` for the one-page brief.",
            "2. Read `output/checkpoint_delta.md` for the change summary since the last frozen checkpoint.",
            "3. Read `legacy/research/notes/examiner-risk-register.md` for the strongest remaining attack points and mitigations.",
            "4. Open `legacy/thesis/thesis.pdf` for the defended manuscript.",
            ...
            "7. Use `legacy/research/notes/what-this-thesis-does-not-claim.md` to keep scope calibration explicit.",
        ],
        [
    ...
    write_readme(
        ...
        metadata,
        [
            "1. Open `legacy/thesis/thesis.pdf` for the submission manuscript.",
            ...
            "4. Read `legacy/research/notes/examiner-risk-register.md` for the calibrated weakness map.",
            "5. Use `output/claim_verification.md` to confirm the thesis-facing evidence spine.",
            "6. Use `output/claim_evidence_map.md` for a compact claim-to-evidence bridge.",
            "7. Use `legacy/thesis/thesis.tex` plus `legacy/thesis/figures/` as the authoritative source bundle.",
        ],
        [
