...
DIST_ROOT = REPO_ROOT / "legacy" / "research" / "review-rounds" / "dist"
...
def ensure_supervisor_packet() -> None:
    if SUPERVISOR_ZIP.exists():
        return
    subprocess.run([resolve_python(), "legacy/scripts/11_package_review_packets.py"], cwd=REPO_ROOT, check=True)
