...
def classify_path(path: str) -> str:
    ...
    if path.startswith("legacy/thesis/") or path.startswith("output/") or path.startswith("code/analysis/05") or path.startswith("code/analysis/06") or path.startswith("code/analysis/07") or path.startswith("code/analysis/08"):
        return "Thesis and empirical core"
    if path.startswith("legacy/research/notes/") or path.startswith("legacy/research/slides/") or path.startswith("legacy/research/packets/"):
        return "Supervisor and defense materials"
    if path.startswith(".github/") or path.startswith("legacy/research/review-rounds/") or path.startswith("legacy/scripts/") or path in {"README.md", "ARCHITECTURE.md", "AGENTS.md", "package.json", ".gitignore", ".node-version"}:
        return "Workflow and repo operations"
    return "Other"
