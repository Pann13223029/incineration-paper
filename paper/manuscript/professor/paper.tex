...
\makeatletter
% Keep dedicated figure pages aligned to the top text margin instead of
% vertically centering a lone float in the printable area.
\setlength{\@fptop}{0pt}
...
$S^{\mathrm{common}}=S_{2005}\cap S_{2024}$. Endpoint-common prevalence is

\begin{equation}
P_t^{K,\mathrm{common}}=
\frac{\sum_{i\in S^{\mathrm{common}}}I_{it}^{K}}
{|S^{\mathrm{common}}|},\qquad t\in\{2005,2024\}.
\label{eq:endpoint-common}
\end{equation}
...
parameters for 35 broad-frame events. An internal model-decision memo documented
this specification before revised estimates were compared. It was not externally
preregistered or prospectively registered; the memo establishes the internal
revision sequence, not confirmatory status. The earlier 11-parameter model is retained as sensitivity.
...
The revision-frozen 300-versus-100 contrast is retained for continuity with the
model decision record, but its empirical support is explicit. In the broad risk frame, 24,
...
$V_{it}$ contains reported facility start-year cohorts before 1990,
1990--1999, and 2000--2009 relative to 2010 or later. Reported facility start
year is an administrative cohort marker, not a verified boiler or generator
...
A direct gross-output model replaces $\log C$ with $\log W$ and $\log K$,
retaining cohort, technology, furnace-count, and year terms.
...
method-matched comparison. Four additional
component regressions therefore use exactly the same design matrix $X_{it}$:
cohort indicators, log processing capacity, furnace count, coarse furnace and
facility types, and fiscal-year indicators. They estimate
...
\end{aligned}
\label{eq:common-control-components}
\end{equation}
...
\label{eq:common-control-identity}
\end{equation}
...
above it. It is retained as a memo-continuity and tail-sensitivity contrast, not
...
diagnostics with model-based intervals; only the revision-frozen primary uses the
...
\midrule
Revision-frozen five-parameter broad frame & 35 & 6.72 (4.31--12.46) & Primary;
...
0.68 to 3.29 entries per 1,000 facility-years. The revision-frozen
...
1,000 facility-years across the support-rich 24--120 t/day range. The revision-frozen
...
\addcontentsline{toc}{section}{Research Transparency}
...
\subsection*{Use of generative artificial intelligence (AI) and AI-assisted
technologies}
...
three principal research questions without consulting external files.
