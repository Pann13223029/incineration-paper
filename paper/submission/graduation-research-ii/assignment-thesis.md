...
net export or revenue. Each workbook is identified by its filename, byte size, parsed sheet, detected header row, field mapping, and SHA-256 hash. The original download timestamps were not recorded, so the hashes identify the files used in this study but do not reconstruct the publisher's full file history.
...
sensitivity resource rather than evidence of independent validation. This limitation remains explicit throughout the analysis.
...
the specification has five parameters for 35 broad-frame events. The five-parameter specification was selected to remain proportionate to the 35 available events. An expanded 11-parameter model is retained as a sensitivity rather than treated as the primary model. Technology and geography are not added because 35 events cannot
...
The 300-versus-100 contrast is retained as an upper-tail sensitivity comparison, and its limited empirical support is made explicit. In the broad risk
...
analyses. The expanded 11-parameter Firth model and conventional logit and
...
whole-lineage bootstrap interval for the primary model. They test
...
predefined upper-tail sensitivity contrast, not treated as the only or most
...
association, not a general equipment-age response. The expanded 11-parameter
...
the primary result uses the 1,999-lineage bootstrap as its headline
uncertainty.
...
|:--|--:|:--|:--|
| Primary five-parameter broad frame | 35 events | 6.72 (4.31-12.46) | Primary; 1,999-lineage bootstrap |
...
baseline model without generator design intensity, the reported-age
...
entries per 1,000 facility-years. The primary 300-versus-100 t/day odds
...
24-120 t/day range. The primary 300-versus-100 t/day odds ratio is 6.72, but
...
## B.1 Technical formulas moved from the main narrative
...
## B.3 Engineering outcome models
...
questions within this document.
